...
Action Input: the input to the action.
              - IMPORTANT: If the tool takes a single, simple argument (e.g., 'errorcode: int'), provide ONLY the raw value.
                Example for 'store_in_audit_db(errorcode: int)': Action Input: 935
              - IMPORTANT: If the tool takes multiple arguments or complex types (e.g., 'list[str]'), provide a JSON dictionary for all arguments.
                Example for 'report_file_creator(errorcode: int, required_fields: list[str])': Action Input: {{"errorcode": 935, "required_fields": ["claim_id", "status"]}}
...
5. STRICTLY adhere to the 'Action Input' format specified above for each tool.
...
Thought:{agent_scratchpad}"""
    ...
    def execute_step(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            if result.get("no_suitable_tool"):
                print(f"🛑 No suitable tool found for step '{current_step_id}'. Ending workflow.")
                state["execution_status"] = "failed"
    ...
    def _intelligent_tool_execution(self, step: Dict[str, Any], state: AgentState) -> Dict[str, Any]:
        ...
        try:
            print(f"🤖 ReAct Agent analyzing step: {step_name}")
            ...
            print(f"❌ Error in intelligent tool execution: {str(e)}")
            return {
                "status": "failure",
                "error": str(e),
    ...
    def _intelligent_condition_evaluation(self, step: Dict[str, Any], state: AgentState) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"❌ Error in intelligent condition evaluation: {str(e)}")
    ...
    def _intelligent_next_step_decision(self, current_step: Dict[str, Any], result: Dict[str, Any], state: AgentState) -> str:
        ...
        try:
            ...
            print(f"⚠️  Error in intelligent next step decision: {str(e)}")
...
if __name__ == "__main__":
    ...
    try:
        print("🚀 Starting Enhanced Multi-Agent Workflow with Intelligent Execution...")
        print("=" * 70)
        ...
        if result.get('execution_log'):
            ...
            for i, log_entry in enumerate(result['execution_log'], 1):
                ...
                if step_result.get('no_suitable_tool'):
                    print(f"   ⚠️  No suitable tool found - workflow terminated")
