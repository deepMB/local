...
def planner(state: AgentState) -> AgentState:
    """Creates a plan based on the task and stores it in the plan field."""
    task = state["task"]
    ...
    try:
        ...
        # Return the plan in the plan field, not as a message
        return {"plan": plan_json}
        
    except Exception as e:
        print(f"Error parsing plan: {e}")
        print("Planning failed. Ending process.")
        # Set error code to indicate planning failure
        return {"error_code": 1, "plan": {"error": "Planning failed"}}

def should_continue_after_planner(state: AgentState) -> str:
    """Determine if we should continue to LLM or end after planning."""
...
Plan: {plan.get('name', 'No plan available')}
Description: {plan.get('description', 'No description available')}

Steps to follow:
{chr(10).join([f"{i+1}. {step['name']}: {step['objective']}" for i, step in enumerate(plan.get('steps', []))]) if plan.get('steps') else 'No steps defined'}
...
"""
    ...
    if not state["messages"]:
        initial_message = HumanMessage(content=f"Please execute the task: {task}")
        messages = [SystemMessage(content=react_system_prompt), initial_message]
    else:
...
# Add edges - ReAct pattern: planner -> (llm or END) -> (tools or END)
    ...
    for step_output in agent.stream(initial_state, thread):
        ...
        if 'plan' in node_output and node_output['plan']:
            ...
            if 'error' in plan:
                ...
                print(f"Plan created: {plan.get('name', 'Unknown')}")
                print(f"Description: {plan.get('description', 'No description')}")
                if plan.get('steps'):
                    print("Steps:")
                    for i, step in enumerate(plan['steps']):
                        print(f"  {i+1}. {step.get('name', 'Unknown step')}: {step.get('objective', 'No objective')}")
        
        if 'error_code' in node_output and node_output['error_code'] != 0:
            print(f"Error occurred. Error code: {node_output['error_code']}")
